...
import { formats, FileFormat } from "../../enums/file.enum";
...
const StyledRepairButton = styled(StyledBottomBarItem)<{ $blink?: boolean }>`
  ${({ $blink, theme }) =>
    $blink
      ? `
    animation: blink 1.5s ease-in-out infinite;
    background: ${theme.TOOLBAR_BG === "#ECECEC" ? "rgba(79, 70, 229, 0.1)" : "rgba(79, 70, 229, 0.2)"};
    color: #4F46E5;
    font-weight: 600;
...
export const BottomBar = () => {
  ...
  React.useEffect(() => {
    if (data?.name) window.document.title = `${data.name} | JSON Repair`;
  }, [data]);
  ...
  return (
    <StyledBottomBar>
      <StyledLeft>
        ...
        {error && currentFormat === FileFormat.JSON && (
          <StyledRepairButton
            $blink={true}
            onClick={async () => {
              await repairJson();
          ...
          <Menu.Dropdown>
            {formats.map((format) => (
              <Menu.Item
                ...
                rightSection={
                  currentFormat === format.value && <IoMdCheckmark />
                }
              >
                {format.label}
              </Menu.Item>
